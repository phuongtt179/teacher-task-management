...
app.post('/api/chat', express.json(), async (req, res) => {
  try {
    const { uid, displayName, messages, channelId } = req.body || {};
 ...
 */
app.post('/api/chat/forward-task', express.json(), async (req, res) => {
  try {
    const { uid, displayName, title, description, priority, deadline, schoolYearId, targetUids, targetNames } = req.body || {};
    if (!uid || !title || !description || !deadline || !schoolYearId || !Array.isArray(targetUids) || targetUids.length === 0) {
      return res.status(400).json({ error: 'missing_fields' });
    }
 ...
 */
app.post('/api/chat/update-profile', express.json(), async (req, res) => {
  try {
    const { uid, displayName } = req.body || {};
    const newName = String(displayName || '').trim();
    if (!uid || !newName) return res.status(400).json({ error: 'missing_fields' });
 ...
 */
app.post('/api/chat/add-school-info', express.json(), async (req, res) => {
  try {
    const { uid, displayName, topic, content, schoolYearId, existingId } = req.body || {};
    if (!uid || !topic || !content) return res.status(400).json({ error: 'missing_fields' });
