...
const CHAT_TOOLS_BASE_DECLARATIONS = [
    ...
    {
      name: 'list_assignable_people',
      description: 'Lấy danh sách TOÀN BỘ người có thể được giao việc (giáo viên, tổ trưởng, hiệu phó, hiệu trưởng, nhân viên), kèm tên và tổ chuyên môn. Dùng khi cần XÁC ĐỊNH đúng người/tổ trước khi giao việc hoặc sửa phân công — ví dụ giao việc cho "cô Lan" hay "tổ Toán", hãy gọi hàm này trước để tự khớp đúng tên/tổ trong danh sách (khớp theo Ý NGHĨA, không cần trùng chữ tuyệt đối), rồi mới lấy đúng uid để gọi confirm_create_task hoặc confirm_edit_task_assignees.',
      parameters: { type: 'OBJECT', properties: {} },
    },
    {
      name: 'confirm_create_task',
      description: 'CHỈ dành cho admin/hiệu phó/hiệu trưởng: chuẩn bị bản nháp GIAO 1 công việc mới cho 1 hoặc nhiều người (không qua file, dùng khi giao việc trực tiếp bằng lời). PHẢI gọi list_assignable_people trước để lấy đúng uid/tên người nhận — KHÔNG tự bịa uid. Chỉ trả về bản nháp để xác nhận, KHÔNG tự ghi vào hệ thống. Nếu người gọi không có quyền, trả lỗi not_authorized.',
      parameters: {
        type: 'OBJECT',
        properties: {
          title: { type: 'STRING', description: 'Tiêu đề công việc' },
          description: { type: 'STRING', description: 'Nội dung/yêu cầu chi tiết công việc' },
          deadline: { type: 'STRING', description: 'Hạn nộp dạng YYYY-MM-DD nếu người giao có nói rõ, để trống nếu không nói' },
          priority: { type: 'STRING', description: 'Mức ưu tiên: low, medium, hoặc high. Để trống nếu không rõ, mặc định medium.' },
          assigneeUids: { type: 'ARRAY', items: { type: 'STRING' }, description: 'Danh sách uid người nhận việc, lấy từ list_assignable_people' },
          assigneeNames: { type: 'ARRAY', items: { type: 'STRING' }, description: 'Tên tương ứng từng uid trong assigneeUids, cùng thứ tự' },
        },
        required: ['title', 'description', 'assigneeUids', 'assigneeNames'],
      },
    },
    {
      name: 'find_tasks_to_edit',
      description: 'CHỈ dành cho admin/hiệu phó/hiệu trưởng: tìm công việc theo tên để chuẩn bị SỬA danh sách người được giao (thêm/bớt người). Gọi trước khi confirm_edit_task_assignees để xác định đúng taskId.',
      parameters: {
        type: 'OBJECT',
        properties: {
          keyword: { type: 'STRING', description: 'Tên hoặc 1 phần tên công việc cần tìm' },
        },
        required: ['keyword'],
      },
    },
    {
      name: 'confirm_edit_task_assignees',
      description: 'CHỈ dành cho admin/hiệu phó/hiệu trưởng: chuẩn bị bản nháp THÊM và/hoặc BỚT người trong danh sách được giao của 1 task đã có (lấy taskId từ find_tasks_to_edit). Nếu thêm người mới, PHẢI gọi list_assignable_people trước để lấy đúng uid — KHÔNG tự bịa. Chỉ trả về bản nháp để xác nhận, KHÔNG tự ghi vào hệ thống.',
      parameters: {
        type: 'OBJECT',
        properties: {
          taskId: { type: 'STRING', description: 'id công việc, lấy từ find_tasks_to_edit' },
          addUids: { type: 'ARRAY', items: { type: 'STRING' }, description: 'uid người muốn thêm vào, lấy từ list_assignable_people — để trống nếu không thêm ai' },
          addNames: { type: 'ARRAY', items: { type: 'STRING' }, description: 'Tên tương ứng từng uid trong addUids, cùng thứ tự' },
          removeUids: { type: 'ARRAY', items: { type: 'STRING' }, description: 'uid người muốn bỏ khỏi việc này (lấy từ currentAssignees/assignedTo đã biết) — để trống nếu không bớt ai' },
        },
        required: ['taskId'],
      },
    },
    ...
    },
    {
      ...
      parameters: {
        type: 'OBJECT',
        properties: {
          taskKeyword: { type: 'STRING', description: 'Tên hoặc 1 phần tên công việc cần tìm bài nộp' },
          teacherName: { type: 'STRING', description: 'Tên giáo viên để lọc, nếu có nhắc tới — để trống nếu chấm cho cả lớp/nhiều người' },
        },
        required: ['taskKeyword'],
      },
    },
    {
      name: 'confirm_grade_submission',
      description: 'CHỈ dành cho admin/hiệu phó/hiệu trưởng: chuẩn bị bản nháp CHẤM ĐIỂM + nhận xét cho 1 bài nộp (lấy submissionId từ find_submissions_for_grading). Điểm phải trong khoảng 0 đến maxScore đã biết. Chỉ trả về bản nháp để xác nhận, KHÔNG tự ghi vào hệ thống.',
      parameters: {
        type: 'OBJECT',
        properties: {
          submissionId: { type: 'STRING', description: 'id bài nộp, lấy từ find_submissions_for_grading' },
          score: { type: 'NUMBER', description: 'Điểm chấm, trong khoảng 0 đến maxScore' },
          feedback: { type: 'STRING', description: 'Nhận xét kèm theo, để trống nếu không có' },
        },
    ...
    },
    {
      ...
      description: 'CHỈ dành cho hiệu trưởng/hiệu phó: xem ĐIỂM SỐ của giáo viên/nhân viên toàn trường (khác get_task_completion_summary là theo dõi HOÀN THÀNH, còn đây là theo dõi ĐIỂM). Không truyền keyword khi hỏi tổng quát kiểu "điểm trung bình toàn trường", "giáo viên nào điểm thấp" — trả về điểm trung bình từng người, sắp xếp thấp→cao để dễ thấy ai cần lưu ý. Có truyền keyword (tên việc) khi hỏi điểm của 1 việc cụ thể kiểu "điểm việc X của mọi người thế nào". Nếu không có quyền, trả lỗi not_authorized.',
      parameters: {
        type: 'OBJECT',
        properties: {
          keyword: { type: 'STRING', description: 'Tên hoặc 1 phần tên công việc cụ thể — để trống nếu hỏi tổng quát' },
        },
      },
    },
    {
    ...
    {
      name: 'confirm_report_progress',
      description: 'Chuẩn bị bản nháp BÁO TIẾN ĐỘ (việc chưa xong, đang làm dở) cho 1 công việc được giao cho CHÍNH giáo viên đang hỏi — dùng khi giáo viên nói kiểu "việc X tôi làm được 50% rồi", "đang làm dở việc Y". PHẢI lấy taskId từ list_my_tasks trước, KHÔNG tự bịa. Chỉ trả về bản nháp để xác nhận, KHÔNG tự ghi vào hệ thống.',
      parameters: {
        type: 'OBJECT',
        properties: {
          taskId: { type: 'STRING', description: 'id công việc, lấy từ list_my_tasks' },
          percent: { type: 'NUMBER', description: 'Mức độ hoàn thành ước lượng 0-100' },
          note: { type: 'STRING', description: 'Ghi chú thêm về tiến độ, có thể để trống' },
        },
        required: ['taskId', 'percent'],
      },
    },
    {
      name: 'confirm_report_blocker',
      description: 'Chuẩn bị bản nháp BÁO VƯỚNG MẮC cho 1 công việc của CHÍNH giáo viên đang hỏi — dùng khi giáo viên nói bị kẹt/chưa làm được vì một lý do (vd "việc X tôi chưa làm được vì thiếu số liệu từ tổ 2"). PHẢI lấy taskId từ list_my_tasks trước. Chỉ trả về bản nháp để xác nhận, KHÔNG tự ghi vào hệ thống.',
      parameters: {
        type: 'OBJECT',
        properties: {
          taskId: { type: 'STRING', description: 'id công việc, lấy từ list_my_tasks' },
          note: { type: 'STRING', description: 'Mô tả vướng mắc/lý do chưa làm được' },
        },
        required: ['taskId', 'note'],
      },
    },
    {
      name: 'confirm_request_extension',
      description: 'Chuẩn bị bản nháp XIN GIA HẠN 1 công việc của CHÍNH giáo viên đang hỏi — dùng khi giáo viên muốn dời hạn nộp (vd "cho tôi xin gia hạn việc X đến 25/11 vì bận công tác"). PHẢI lấy taskId từ list_my_tasks trước. requestedDeadline dạng YYYY-MM-DD. Yêu cầu này cần Ban Giám Hiệu duyệt. Chỉ trả về bản nháp để xác nhận, KHÔNG tự ghi vào hệ thống.',
      parameters: {
        type: 'OBJECT',
        properties: {
          taskId: { type: 'STRING', description: 'id công việc, lấy từ list_my_tasks' },
          requestedDeadline: { type: 'STRING', description: 'Hạn mới mong muốn dạng YYYY-MM-DD' },
          note: { type: 'STRING', description: 'Lý do xin gia hạn' },
        },
        required: ['taskId', 'requestedDeadline', 'note'],
      },
    },
    {
      name: 'confirm_request_add_person',
      description: 'Chuẩn bị bản nháp ĐỀ XUẤT BỔ SUNG NGƯỜI vào 1 công việc của CHÍNH giáo viên đang hỏi — dùng khi giáo viên thấy việc quá tải/cần thêm người và muốn đề nghị Ban Giám Hiệu giao thêm ai đó (vd "việc X nên giao thêm cô B phụ giúp"). Giáo viên KHÔNG tự thêm người được, chỉ GỬI ĐỀ XUẤT để BGH quyết định. PHẢI lấy taskId từ list_my_tasks trước. Chỉ trả về bản nháp để xác nhận, KHÔNG tự ghi vào hệ thống.',
      parameters: {
        type: 'OBJECT',
        properties: {
          taskId: { type: 'STRING', description: 'id công việc, lấy từ list_my_tasks' },
          note: { type: 'STRING', description: 'Nên bổ sung ai và vì sao (vd "giao thêm cô B vì khối lượng lớn")' },
        },
        required: ['taskId', 'note'],
      },
    },
...
async function toolListMyTasks(uid) {
  ...
  // Cập nhật giữa chừng MỚI NHẤT của giáo viên cho từng công việc (tiến độ/vướng mắc/gia hạn)
  const latestUpdateByTask = new Map();
  updatesSnap.docs.forEach(d => {
    const u = d.data();
    const at = u.createdAt?.toMillis ? u.createdAt.toMillis() : 0;
    const cur = latestUpdateByTask.get(u.taskId);
    if (!cur || at > cur._at) latestUpdateByTask.set(u.taskId, { ...u, _at: at });
  });
  ...
  const tasks = tasksSnap.docs.map(d => {
    ...
    const upd = latestUpdateByTask.get(d.id);
    return {
      id: d.id,
      title: t.title,
      ...
      latestUpdate: upd
        ? { type: upd.type, percent: upd.percent ?? null, status: upd.status || 'open', note: upd.note || null }
        : null,
    };
  });
...
async function toolListAssignablePeople() {
  ...
  deptsSnap.docs.forEach(d => {
    const dept = d.data();
    (dept.memberIds || []).forEach(memberUid => deptByMember.set(memberUid, dept.name));
  });
  const people = usersSnap.docs.map(d => ({
    uid: d.id,
    name: d.data().displayName || d.data().email || d.id,
    department: deptByMember.get(d.id) || null,
  }));
  return { people };
}

// Chuẩn bị bản nháp BÁO TIẾN ĐỘ / VƯỚNG MẮC / XIN GIA HẠN của chính giáo viên cho 1 công việc.
...
async function toolConfirmReportUpdate(uid, args, type) {
  ...
  if (type === 'extension') {
    if (!/^\d{4}-\d{2}-\d{2}$/.test(args?.requestedDeadline || '')) return { error: 'missing_deadline' };
    if (!note) return { error: 'missing_reason' };
    const cur = task.deadline2?.toDate ? task.deadline2.toDate() : (task.deadline?.toDate ? task.deadline.toDate() : null);
    return {
      ...base,
      requestedDeadline: args.requestedDeadline,
      currentDeadline: cur ? cur.toISOString().slice(0, 10) : null,
    };
  }
  // blocker
  if (!note) return { error: 'missing_reason' };
...
async function toolConfirmCreateTask(uid, args) {
  ...
  const title = String(args?.title || '').trim();
  const description = String(args?.description || '').trim();
  const assigneeUids = Array.isArray(args?.assigneeUids) ? args.assigneeUids : [];
  const assigneeNames = Array.isArray(args?.assigneeNames) ? args.assigneeNames : [];
  if (!title || !description) return { error: 'missing_fields' };
...
async function toolFindTasksToEdit(uid, keyword) {
  ...
  const matches = tasksSnap.docs
    ...
    .map(d => {
      const t = d.data();
      const uids = t.assignedTo || [];
      const names = t.assignedToNames || [];
      return {
        taskId: d.id,
        title: t.title,
        // Trả cả uid lẫn tên (cùng thứ tự) — cần uid để gọi removeUids khi bớt người khỏi việc này.
        currentAssignees: uids.map((u, i) => ({ uid: u, name: names[i] || u })),
      };
    })
    .slice(0, 10);
...
async function toolConfirmEditTaskAssignees(uid, args) {
  ...
  const removeUids = Array.isArray(args?.removeUids) ? args.removeUids : [];

  const currentUids = task.assignedTo || [];
  const currentNames = task.assignedToNames || [];
  const currentPairs = currentUids.map((u, i) => ({ uid: u, name: currentNames[i] || u }));

  const afterRemoval = currentPairs.filter(p => !removeUids.includes(p.uid));
  const newPairs = addUids
    .filter(u => !afterRemoval.some(p => p.uid === u))
    .map((u, i) => ({ uid: u, name: addNames[i] || u }));
  const finalPairs = [...afterRemoval, ...newPairs];

  if (finalPairs.length === 0) return { error: 'would_remove_all_assignees' };
  ...
  return {
    ...
    beforeNames: currentNames,
    afterUids: finalPairs.map(p => p.uid),
    afterNames: finalPairs.map(p => p.name),
  };
}
...
async function toolFindSubmissionsForGrading(uid, taskKeyword, teacherName) {
  ...
  const results = subsSnap.docs
    .filter(d => d.data().isLatest !== false)
    .filter(d => !nameKw || (d.data().teacherName || '').toLowerCase().includes(nameKw))
    .map(d => {
      const s = d.data();
      return {
        submissionId: d.id,
        taskId: s.taskId,
        taskTitle: taskById.get(s.taskId)?.title || '',
        maxScore: taskById.get(s.taskId)?.maxScore ?? 10,
        teacherName: s.teacherName,
        content: s.content,
        currentScore: s.score ?? null,
        currentFeedback: s.feedback || null,
      };
    });
...
async function toolConfirmGradeSubmission(uid, args) {
  ...
  if (!Number.isFinite(score) || score < 0 || score > maxScore) {
    return { error: 'invalid_score', maxScore };
  }
  ...
  return {
    ...
    newScore: score, newFeedback: feedback,
  };
}

// Tổng quan ĐIỂM SỐ của giáo viên/nhân viên toàn trường — CHỈ hiệu trưởng/hiệu phó.
...
async function toolGetScoreOverview(uid, keyword) {
  ...
  if (kw) {
    ...
    subsSnap.docs.forEach(d => {
      ...
      list.push({ teacherName: s.teacherName, score: s.score ?? null, feedback: s.feedback || null });
      byTask.set(s.taskId, list);
    });

    const tasks = taskIds.map(tid => ({
      taskTitle: taskById.get(tid)?.title || '',
      maxScore: taskById.get(tid)?.maxScore ?? 10,
      scores: byTask.get(tid) || [],
    }));
  ...
  const scoresByTeacher = new Map();
  subsSnap.docs.forEach(d => {
    const s = d.data();
    if (s.isLatest === false) return;
    if (typeof s.score !== 'number') return;
    const list = scoresByTeacher.get(s.teacherId) || [];
    list.push(s.score);
    scoresByTeacher.set(s.teacherId, list);
  });

  const overview = Array.from(scoresByTeacher.entries()).map(([teacherId, scores]) => ({
    teacherName: nameById.get(teacherId) || teacherId,
    averageScore: Math.round((scores.reduce((a, b) => a + b, 0) / scores.length) * 10) / 10,
    gradedCount: scores.length,
  }));
  overview.sort((a, b) => a.averageScore - b.averageScore);

  return { mode: 'overview', teachers: overview.slice(0, 100) };
}
...
3. Nếu tên nhắc tới không khớp rõ ràng đúng 1 người (trùng tên nhiều người, hoặc không tìm thấy), hỏi lại rõ ràng thay vì đoán đại.
4. Sau khi confirm_create_task trả về confirmed=true, xác nhận lại bằng lời (tiêu đề, người nhận, hạn) và mời bấm nút xác nhận hiện ra trong chat — KHÔNG tự nói "đã giao xong".
Khi admin/hiệu phó/hiệu trưởng muốn SỬA danh sách người được giao của 1 việc đã có (thêm người mới, hoặc bỏ bớt người), ví dụ "phân thêm cô B vào việc X", "bỏ cô A ra khỏi việc X, không giao nữa":
1. Gọi find_tasks_to_edit(keyword) với tên việc để xác định đúng taskId — nếu tìm ra nhiều việc trùng tên, hỏi lại rõ việc nào.
2. Nếu cần THÊM người mới, gọi list_assignable_people trước để lấy đúng uid người muốn thêm.
3. Gọi confirm_edit_task_assignees(taskId, addUids/addNames nếu thêm, removeUids nếu bớt — removeUids lấy từ currentAssignees của find_tasks_to_edit, KHÔNG tự bịa).
4. Sau khi trả về confirmed=true, xác nhận lại bằng lời (danh sách trước/sau) và mời bấm nút xác nhận — KHÔNG tự nói "đã sửa xong".
Khi admin/hiệu phó/hiệu trưởng muốn CHẤM ĐIỂM/phản hồi 1 bài nộp, ví dụ "chấm bài việc X cho cô A 9 điểm, nhận xét làm tốt":
1. Gọi find_submissions_for_grading(taskKeyword, teacherName nếu có) để tìm đúng submissionId — nếu tìm ra nhiều bài trùng khả năng (nhiều giáo viên cùng nộp việc đó mà không rõ chấm cho ai), hỏi lại rõ ràng.
...
Khi giáo viên muốn BÁO CÁO GIỮA CHỪNG về 1 công việc được giao (chưa nộp hẳn, chỉ cập nhật tình hình) — LUÔN gọi list_my_tasks trước (nếu chưa có) để lấy đúng taskId, rồi:
- Báo tiến độ đang làm dở (vd "việc X tôi làm được 50% rồi", "đang làm gần xong"): gọi confirm_report_progress(taskId, percent, note nếu có).
- Báo vướng mắc/lý do chưa làm được (vd "việc X chưa làm được vì thiếu số liệu tổ 2"): gọi confirm_report_blocker(taskId, note).
- Xin gia hạn/dời hạn nộp (vd "cho tôi xin gia hạn việc X đến 25/11 vì bận"): gọi confirm_request_extension(taskId, requestedDeadline dạng YYYY-MM-DD, note lý do). Nhắc rõ yêu cầu này cần Ban Giám Hiệu duyệt.
- Đề xuất bổ sung thêm người vào việc (vd "việc X nên giao thêm cô B phụ giúp", "việc này cần thêm người"): gọi confirm_request_add_person(taskId, note nêu nên bổ sung ai + vì sao). Nhắc rõ giáo viên không tự thêm được, chỉ gửi đề xuất để BGH quyết định.
Sau khi hàm trả về confirmed=true, xác nhận lại bằng lời (việc gì, nội dung báo cáo) và mời giáo viên bấm nút xác nhận hiện ra trong chat — KHÔNG tự nói "đã gửi/đã báo xong", việc ghi chỉ xảy ra khi giáo viên tự bấm xác nhận. Nếu hàm trả lỗi not_assigned thì báo là việc đó không được giao cho họ; nếu thiếu hạn/lý do (missing_deadline/missing_reason) thì hỏi lại cho đủ.
...
Khi giáo viên hỏi CÁCH DÙNG app (không phải hỏi dữ liệu cụ thể), trả lời dựa vào phần HƯỚNG DẪN SỬ DỤNG APP ở trên — KHÔNG cần gọi hàm nào.`;
    ...
    let createTaskCandidateForUI = null;
    let editTaskAssigneesCandidateForUI = null;
    let gradeSubmissionCandidateForUI = null;
    let reportUpdateCandidateForUI = null;
    ...
    for (let round = 0; round < 5; round++) {
      ...
      for (const fc of functionCalls) {
        ...
        if (fc.name === 'confirm_create_task' && result.confirmed) {
          createTaskCandidateForUI = result;
        }
        if (fc.name === 'confirm_edit_task_assignees' && result.confirmed) {
          editTaskAssigneesCandidateForUI = result;
        }
        if (fc.name === 'confirm_grade_submission' && result.confirmed) {
          gradeSubmissionCandidateForUI = result;
        }
        if (
          (fc.name === 'confirm_report_progress' ||
            fc.name === 'confirm_report_blocker' ||
            fc.name === 'confirm_request_extension' ||
            fc.name === 'confirm_request_add_person') &&
          result.confirmed
        ) {
    ...
    if (createTaskCandidateForUI) responseBody.createTaskCandidate = createTaskCandidateForUI;
    if (editTaskAssigneesCandidateForUI) responseBody.editTaskAssigneesCandidate = editTaskAssigneesCandidateForUI;
    if (gradeSubmissionCandidateForUI) responseBody.gradeSubmissionCandidate = gradeSubmissionCandidateForUI;
    if (reportUpdateCandidateForUI) responseBody.reportUpdateCandidate = reportUpdateCandidateForUI;
